/* ===================================================================
   MESLEK PUSULASI — Veri modeli (içerik)
   Omurga: RIASEC (Holland) mesleki ilgi + Beş Faktör kısa karakter aynası
   Dünya çapı araştırma brifine dayalı. v2.
   Tüm hesaplama tarayıcıda yapılır; hiçbir veri sunucuya gönderilmez.
   =================================================================== */
window.MP = (function () {
  "use strict";
  ...
  /* ---------- RIASEC tip tanımları ---------- */
  const types = {
    R: { key:"R", name:"Yapıcı", classic:"Gerçekçi", emoji:"🛠️", hex:"#E8743B",
         tagline:"Eliyle iş yapan, üreten, çözen.",
         onet:"Somut, pratik, elle yapılan işlerden; araç, makine, doğa ve sahadan keyif alır.",
         blurb:"Somut, elle tutulur işler senin alanın. Bir şeyi tamir etmek, kurmak, çalıştırmak; doğada, sahada, atölyede olmak sana iyi gelir.",
         strengths:["Pratik zekâ","El becerisi","Dayanıklılık","Çözüm üretme"] },
    I: { key:"I", name:"Araştırmacı", classic:"Araştırıcı", emoji:"🔬", hex:"#1B6CA8",
         ...
         blurb:"“Neden?” ve “nasıl?” soruları senin oyun alanın. Verilerle, bilmecelerle uğraşmak ve derinlemesine düşünmek seni canlandırır.",
    ...
    S: { key:"S", name:"Yardımsever", classic:"Sosyal", emoji:"🤝", hex:"#27AE60",
         ...
         blurb:"İnsanlara faydalı olmak seni mutlu eder. Öğretmek, iyileştirmek, dinlemek ve bir arada çalışmak senin gücün.",
         strengths:["Empati","İletişim","Öğretme","İş birliği"] },
    E: { key:"E", name:"Girişimci", classic:"İkna Edici", emoji:"🚀", hex:"#E74C3C",
         ...
         blurb:"İnisiyatif almayı, ikna etmeyi, bir işi başlatıp yönetmeyi seversin. Hedef koyup peşinden gitmek sana heyecan verir.",
         strengths:["Liderlik","İkna","Girişimcilik","Kararlılık"] },
    C: { key:"C", name:"Düzenleyici", classic:"Geleneksel", emoji:"🗂️", hex:"#34495E",
         ...
         blurb:"Düzen, kesinlik ve sistem seni rahatlatır. Verileri toparlamak, planlamak, işleri kusursuz yürütmek senin tarzın.",
         strengths:["Düzen","Dikkat","Güvenilirlik","Sistematiklik"] }
  };

  /* ---------- Beş Faktör (Big Five) ----------
     Depolama: ham boyutlar E,A,C,N,O. SUNUM: N yerine "Duygusal Denge" (=100-N).
     poles: [düşük etiket, yüksek etiket] — nötr, damgalamayan.
     fb: yüksek/orta/düşük geri bildirim (Barnum önleme). ES için yüksek=dengeli.
  */
  const bigfive = {
    ...
    C:  { key:"C",  name:"Sorumluluk & Düzen", orig:"Conscientiousness", emoji:"🎯", display:true,
          poles:["Esnek-Spontane","Planlı-Düzenli"],
          strengths:["Disiplin","Güvenilirlik"],
    ...
    E:  { key:"E",  name:"Dışadönüklük & Enerji", orig:"Extraversion", emoji:"⚡", display:true,
          poles:["Sakin-Odaklı","Enerjik-Sosyal"],
          strengths:["İletişim","Enerji"],
          fb:{ high:"Sosyal ve enerjiksin; kalabalıklar, yeni insanlar ve hareket seni canlandırır.",
               mid:"Hem sosyalleşmekten keyif alıyor hem de kendi alanına çekilmeye ihtiyaç duyuyorsun — ikisini de dengeliyorsun.",
               low:"Sakin ve odaklısın; az kişiyle derin bağlar ve kendi alanın sana iyi gelir." } },
    A:  { key:"A",  name:"Uyumluluk & Sıcaklık", orig:"Agreeableness", emoji:"💛", display:true,
          poles:["Bağımsız-Eleştirel","Şefkatli-Uyumlu"],
          strengths:["Empati","Takım ruhu"],
          fb:{ high:"Şefkatli ve uyumlusun; başkalarının duygularını önemser, iş birliğini rekabete tercih edersin.",
               ...
               low:"Bağımsız ve eleştirelsin; kendi fikrini savunur, kararlarını başkalarına göre değil mantığına göre verirsin." } },
    N:  { key:"ES", name:"Duygusal Denge & Sükûnet", orig:"Emotional Stability", emoji:"🧘", display:true, fromN:true,
          poles:["Hassas-Tepkisel","Sakin-Dengeli"],
          strengths:["Soğukkanlılık","Dayanıklılık"],
          fb:{ high:"Baskı altında sakin kalabiliyorsun; aksiliklere karşı dayanıklısın, kolay kolay paniğe kapılmazsın.",
  ...
  /* ---------- Maddeler ----------
     format: likert | swipe | slider | scenario
     layer:  riasec | bigfive | values | mixed
     type:   RIASEC harfi (riasec/swipe)
     dim:    O|C|E|A|N (bigfive); vkey (values)
     scale:  riasec | bigfive | values
     reverse: bigfive ters madde
  */
  const items = [
    /* ===== TUR 1: R + I ===== */
    ...
    { id:"SJT1", turn:1, format:"scenario", layer:"mixed",
      text:"Bir hafta sonu boş kaldın. Hangisi sana en cazip geliyor?",
      hint:"Sana en yakın olanı seç.",
      options:[
    ...
    { id:"SJT2", turn:2, format:"scenario", layer:"mixed",
      text:"Grup projesinde son gün her şey karıştı. İlk içinden gelen ne olur?",
      hint:"Spontane tepkini seç — doğrusu yok.",
      options:[
    ...
    { id:"V_IN1", turn:5, format:"likert", scale:"values", layer:"values", vkey:"IN", text:"Bana sürekli ne yapacağımın söylenmesindense kendi kararlarımı vermek." },
  ...
  /* Big Five ters maddeleri (referans) */
  const BIGFIVE_REVERSE = ["E_3", "E_4", "A_3", "C_3", "C_4", "N_3", "N_4"];
  ...
  */
  const archetypes = [
    ...
    { id:3,  ad:"Sessiz Mucit",        emoji:"🔬", hex:"#2E86AB", primary:"I", bf:["C","E-"],
    ...
    { id:7,  ad:"Şefkatli Rehber",     emoji:"🤝", hex:"#27AE60", primary:"S", bf:["A"],
      def:"Başkalarının nasıl hissettiğini hemen anlıyorsun; yardım etmek sana iyi geliyor. Yanında insanlar güvende hissediyor." },
    { id:8,  ad:"İlham Veren Öğretmen",emoji:"📣", hex:"#16A085", primary:"S", bf:["E"],
      def:"Bir şeyi anlatıp birinin gözünde ışık yakmak senin işin. İnsanları bir araya getirmeyi seviyorsun." },
    { id:9,  ad:"Doğal Lider",         emoji:"🚀", hex:"#E74C3C", primary:"E", bf:["E"],
      def:"İnsanları bir hedefe taşımak sana enerji veriyor. Ortam karıştığında herkes farkında olmadan sana dönüyor." },
    { id:10, ad:"Vizyoner Girişimci",  emoji:"💡", hex:"#D35400", primary:"E", bf:["O"],
      def:"Aklında hep yeni bir fikir, kurulacak bir iş var. Fırsatları başkalarından önce görüyorsun." },
    { id:11, ad:"Düzen Ustası",        emoji:"🗂️", hex:"#34495E", primary:"C", bf:["C"],
      def:"Karmaşayı düzene çevirmek, plan yapmak, işleri tıkır tıkır yürütmek senin işin. Sana hedef ver, haritayı sen çizersin." },
    { id:12, ad:"Güven Veren Analist",  emoji:"📊", hex:"#2C3E50", primary:"C", bf:["ES"],
      def:"Sakin kafanla rakamları, detayları kontrol edersin. Kimsenin gözden kaçırdığı hatayı sen bulursun." },
    { id:13, ad:"Çok Yönlü Köprü",     emoji:"🧭", hex:"#7F8C8D", special:"lowdiff",
      def:"Birçok alana eşit ilgin var — bu bir kararsızlık değil, zenginlik. Farklı dünyaları birbirine bağlayabilirsin." },
  ...
  /* primary harf + belirgin BF -> arketip id (yoksa kök) */
  const archetypeMap = {
    R:{ O:1, ES:1, C:2, _:1 },
    I:{ C:3, "E-":3, O:4, _:4 },
    A:{ O:5, E:6, _:5 },
  ...
  /* Arketip-sohbet köprü soruları (her arketip) */
  const archetypeChat = {
  ...
  /* ---------- Türkiye meslek/bölüm kataloğu ----------
     kod: Holland 3 harf (vektör app.js'te üretilir). jobZone: 2-5 (eğitim düzeyi)
  */
  const careers = [
    // R
    ...
    { ad:"Siber Güvenlik", kod:"ICR", puanTuru:"SAY/TYT", jobZone:4, trend:true, ornek:["Siber güvenlik uzmanı","Ağ güvenliği analisti"],
      why:"Merak, sistematik takip ve dijital problem çözmeyi birleştirir.",
      next:"Linux ve ağ temellerine bak; yasal/etik CTF platformlarında başlangıç seviyesi denemeler yap.",
      watchout:"Bu alan etik sorumluluk, disiplin ve sürekli güncel kalma ister." },
    { ad:"Moleküler Biyoloji ve Genetik", kod:"IRA", puanTuru:"SAY", jobZone:5, trend:true, ornek:["Genetik araştırmacı","Biyomedikal uzmanı"] },
    { ad:"Biyomedikal Mühendisliği / Cihaz Teknolojisi", kod:"RIS", puanTuru:"SAY/TYT", jobZone:4, trend:true, ornek:["Biyomedikal mühendisi","Tıbbi cihaz teknikeri"],
      why:"Teknik sistemleri sağlık alanındaki insan faydasıyla buluşturur.",
      next:"Bir tıbbi cihazın nasıl çalıştığını araştır; sensörler ve medikal teknoloji hakkında mini dosya hazırla.",
      watchout:"Teknik bilgi, sağlık güvenliği ve mevzuat birlikte önemlidir." },
    ...
    { ad:"Endüstri Mühendisliği", kod:"ICE", puanTuru:"SAY", jobZone:4, trend:true, ornek:["Süreç analisti","Operasyon uzmanı"],
      why:"Veri, sistem kurma, verimlilik ve insan koordinasyonunu aynı masaya getirir.",
      next:"Okulda veya evde tekrar eden bir süreci ölç; darboğazı bulup iyileştirme önerisi çıkar.",
      watchout:"Matematiksel düşünme kadar iletişim ve sunum da gerekir." },
    // A
    ...
    { ad:"UX/UI Tasarımı", kod:"ASI", puanTuru:"EA/SÖZ/SAY", jobZone:3, trend:true, ornek:["UX tasarımcı","Arayüz tasarımcısı"],
      why:"İnsan ihtiyacını anlama, estetik ve analiz taraflarını birlikte kullanır.",
      next:"Bir mobil uygulamanın üç ekranını yeniden tasarla; iki kişiye gösterip neyi anlamadıklarını sor.",
      watchout:"Sadece güzel görünüm değil, kullanıcı araştırması ve geri bildirimle çalışmak gerekir." },
    { ad:"Dijital Oyun Tasarımı", kod:"AIE", puanTuru:"SAY/SÖZ/Yetenek", jobZone:4, trend:true, ornek:["Oyun tasarımcısı","Seviye tasarımcısı"],
      why:"Hikaye, estetik, sistem düşüncesi ve teknik üretimi bir araya getirir.",
      next:"Kağıt üstünde küçük bir oyun kuralı tasarla veya basit bir oyun prototipi hazırla.",
    ...
    { ad:"Beslenme ve Diyetetik", kod:"SIC", puanTuru:"SAY", jobZone:4, trend:true, ornek:["Diyetisyen","Toplum sağlığı uzmanı"],
      why:"Bilimsel bilgiyi insanlara uygulanabilir ve destekleyici biçimde aktarmayı gerektirir.",
      next:"Bir beslenme uzmanının bir gününü araştır; bilimsel kaynak ile sosyal medya bilgisini karşılaştır.",
      watchout:"İnsanlarla sürekli iletişim ve kanıta dayalı çalışma birlikte yürür." },
    { ad:"Dil ve Konuşma Terapisi", kod:"SIA", puanTuru:"SAY", jobZone:5, trend:true, ornek:["Dil ve konuşma terapisti"],
      why:"İnsana yardım etme, gözlem, sabır ve bilimsel değerlendirme taraflarını birleştirir.",
      next:"Bu alandaki vaka örneklerini oku; bir uzmandan günlük çalışma akışını öğrenmeye çalış.",
      watchout:"Uzun eğitim, dikkatli gözlem ve duygusal dayanıklılık ister." },
    ...
    { ad:"Dijital Ürün Yönetimi", kod:"EIC", puanTuru:"EA/SAY", jobZone:4, trend:true, ornek:["Ürün yöneticisi","Growth uzmanı"],
      why:"Kullanıcı ihtiyacı, veri, ikna ve ekip koordinasyonunu aynı rotada toplar.",
      next:"Sevdiğin bir uygulama için bir iyileştirme fikri yaz; hedef kullanıcı, veri ve ekran taslağı ekle.",
  ...
  /* Üçlü kod -> kısa profil mesajı (örnek/öne çıkanlar) */
  const codeMessages = {
  ...
  /* ---------- Sohbet / diyalog (statik şablon + değişken) ---------- */
  const dialogue = {
    self: [
  ...
  const texts = {
    ...
    resultTop:"<strong>Senin keşif haritan.</strong> Bu sonuçlar bugünkü ilgilerinin bir fotoğrafı — kesin bir kader değil. Şu alanlara ilgi duyuyor <em>olabilirsin</em>. En iyisi: birkaçını gerçek hayatta dene, öğretmenine/ailene danış. Fikrini değiştirmekte özgürsün.",
    ...
    reliability:"Bu kısa bir keşif testidir; klinik değerlendirme, zeka testi, yetenek sınavı veya kesin meslek kararı değildir. Onu bir başlangıç noktası, bir sohbet açıcı olarak gör. Okul/bölüm gibi önemli kararlarda rehber öğretmenine ya da RAM'a da danış.",
    sources:"Model omurgası Holland/RIASEC mesleki ilgi yaklaşımı, O*NET Interest Profiler, O*NET iş/çalışan içerik modeli ve Beş Faktör kişilik dilinden yararlanır. Bu uygulama lisanslı klinik test, zekâ/yetenek sınavı veya kesin tercih motoru değildir; Türkiye bağlamına uyarlanmış bir keşif ve sohbet başlatma aracıdır."
  };

  return {
    version: "v2", scales, types, bigfive, turns, items, BIGFIVE_REVERSE,
    valueDims, bridge, archetypes, archetypeMap, specialPairs, archetypeChat,
    careers, codeMessages, dialogue, steps, texts,
    RIASEC: ["R","I","A","S","E","C"]
  };
})();
