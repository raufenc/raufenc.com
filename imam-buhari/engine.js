...
function populateHero(){
  ...
  document.getElementById('sh-hadith-title').textContent=SCHOLAR.sections.hadith.title;
  document.getElementById('sh-hadith-desc').textContent=SCHOLAR.sections.hadith.desc;
...
function renderBookContent(){
  ...
  BOOK.filter(ch=>ch.id!=='hadisler').forEach((ch,ci)=>{
...
/* ═══════════ HADİSLER ═══════════ */
function renderHadithContent(){
  const container=document.getElementById('hadith-content');
  const hadithChapter=BOOK.find(ch=>ch.id==='hadisler');
  if(!hadithChapter)return;
  hadithChapter.subs.forEach(s=>{
    const card=document.createElement('div');card.className='hadith-card';
    card.innerHTML=`<div class="hadith-label">Hadîs-i Şerîf</div><div class="sub-title">${s.title}</div><div class="sub-text">${s.text}</div>`;
...
function setupNavHighlight(){
  ...
  const ids=['map-section','timeline-section','content-section','hadisler'];
